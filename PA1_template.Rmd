...
```{r Making a datatable, echo=TRUE, message = FALSE}
...
```
### Daily Steps Plot
```{r Histogram2, echo=TRUE, message = TRUE}
### Histogram Plot
...
```
### Time Series Plot  
```{r Time Series,echo=TRUE, message = TRUE}
...
```{r Historgram3 without nas, echo=FALSE, message = FALSE}
...
datasumnano <- as.data.frame(datatabledaynoNA[, sum(FinSteps, na.rm = TRUE),by = Date])
...
```{r  Weekend vs Weekday, echo=TRUE, message = FALSE}
par(mfrow=c(2,1))
